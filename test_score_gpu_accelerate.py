...
def cal_metric(gt_sp, gt_mask, pr_sp, pr_mask, shape=None):
    ...
    result = {"Image-AUROC": spauc,"Image-Average precision": spap,"Image-F1-maximum": spmaxf1,"Image-AUPR": spaupr,
              ...
              "Pixel-AUPRO@30%":  pro[0][0], "Pixel-AUPRO@10%":  pro[0][1], "Pixel-AUPRO@5%":
                  pro[0][2],"Pixel-AUPRO@1%":  pro[0][3],
              }
    return result
...
if __name__ == '__main__':
    ...
    Image_gt, Image_pre = torch.randint(0, 2, (B,)),  torch.randn( (B,)),
    H,W = gt.shape


    #### numpy to tensor
    gt = torch.from_numpy(gt).cuda().unsqueeze(0).repeat((B, 1, 1))
    pre = torch.from_numpy(pre).cuda().unsqueeze(0).repeat((B, 1, 1))

    metric = cal_metric(Image_gt, gt, Image_pre,pre,shape=(H,W))
    print(metric)